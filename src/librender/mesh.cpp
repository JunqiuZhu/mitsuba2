...
Mesh<Float, Spectrum>::add_attribute(const std::string& name, size_t size) {
    ...
    if (is_vertex_attribute) {
        auto[it, success] = m_mesh_attributes.insert({ name, { size, VERTEX, empty<FloatStorage>(m_vertex_count * size)} });
        return it->second.buf;
    } else {
        auto[it, success] = m_mesh_attributes.insert({ name, { size, FACE, empty<FloatStorage>(m_face_count * size)} });
        return it->second.buf;
    }
}
